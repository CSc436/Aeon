...
package org.interguild.editor.scrollBar {
	...
	public class VerticalScrollBar extends Sprite {
		...
		//============================================================================================================================
		public function VerticalScrollBar(c:DisplayObjectContainer, tc:uint, gc:uint, gpc:uint, grip:uint, tt:int, gt:int, ea:int, hs:Boolean, w:int)
		//============================================================================================================================
		{
			_content=c;
			_trackColor=tc;
		...
		//============================================================================================================================
		{
			...
			if (_content.height < Aeon.STAGE_HEIGHT) {
		...
		//============================================================================================================================
		{
			...
			if (d > 0) {
				...
				if (((_grabber.y + _grabber.height) + (Math.abs(d) * 4)) <= Aeon.STAGE_HEIGHT) {
					_grabber.y+=Math.abs(d) * 4;
				} else {
					_grabber.y=Aeon.STAGE_HEIGHT - _grabber.height;
				}
				if (!_grabber.willTrigger(Event.ENTER_FRAME))
					_grabber.addEventListener(Event.ENTER_FRAME, scrollContent);
			}
		...
		//============================================================================================================================
		{
			...
			if (p < _grabber.y) {
				...
				if (_grabber.y + _grabber.getChildByName("bg").height > _track.height)
					_grabber.y=Aeon.STAGE_HEIGHT - _grabber.getChildByName("bg").height;
			}
